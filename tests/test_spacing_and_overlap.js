...
test('spacing and overlap stay within bounds across status, settings, linked display, and token-driven reflow', async () => {
  ...
  try {
    for (const [label, target, consoleEntries] of [
      ...
      ['settings-display-default', surfaces.settingsDisplayPage, surfaces.settingsDisplayConsole],
    ]) {
    ...
    const settingsDisplayAfterTokens = await captureSnapshot(surfaces.settingsDisplayPage, 'settings-display-after-token-reflow');
    recordSnapshotArtifact('status-after-token-reflow', statusAfterTokens, surfaces.statusConsole, {
      settingsDisplaySnapshot: settingsDisplayAfterTokens,
    });
    ...
    assertNoOverlap(settingsDisplayAfterTokens);
...
test('linked display and settings do not leave screenshot-scale empty regions after default load or auto arrange', async () => {
  const surfaces = await openRealSurfaces(suite, { width: 1280, height: 720 }, 'screenshot-regressions');

  try {
    const settingsDisplayDefault = await captureSnapshot(surfaces.settingsDisplayPage, 'settings-display-screenshot-default');
    const settingsDefault = await captureSnapshot(surfaces.settingsPage, 'settings-screenshot-default');
    recordSnapshotArtifact('screenshot-regressions-default', settingsDisplayDefault, surfaces.settingsDisplayConsole, {
      settingsSnapshot: settingsDefault,
    });

    assert.ok(settingsDisplayDefault.cards.some((card) => card.key === 'setup-hero'), 'Linked display surface is missing the Setup Hero card');
    ...
    const settingsDisplayAuto = await captureSnapshot(surfaces.settingsDisplayPage, 'settings-display-screenshot-auto');
    const settingsAuto = await captureSnapshot(surfaces.settingsPage, 'settings-screenshot-auto');
    recordSnapshotArtifact('screenshot-regressions-auto', settingsDisplayAuto, surfaces.settingsDisplayConsole, {
